...
static struct workqueue_struct *dbs_wq = NULL;
...
/*
 * hp_mutex protects all hotplug related global variables
 */
...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (time_before(next_sampling, appointed_at)) {
			...
			if (dbs_wq == NULL)
				printk(KERN_EMERG "[power/dvfs] %s():%d, impossible\n", __func__, __LINE__);
			else
				queue_delayed_work_on(dbs_info->cpu, dbs_wq, &dbs_info->work, usecs_to_jiffies(new_rate));
		}
		mutex_unlock(&dbs_info->timer_mutex);
	}
}
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	dbs_wq = alloc_workqueue("do_dbs_timer", WQ_HIGHPRI, 0);
